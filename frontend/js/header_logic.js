...
function handleMoreDropdown() {
  const moreJs = document.querySelector(".more-js");
  const moreCont = document.querySelector(".more-container-js");

  moreJs?.addEventListener("click", (e) => {
    if (!moreCont?.contains(e.target)) {
      e.stopPropagation();
      if (moreCont?.classList.contains("inactive")) {
        document.querySelector(".theme-container-js")?.classList.replace("active", "inactive");
        document.querySelector(".settings-container-js")?.classList.replace("active", "inactive");
        moreCont.classList.replace("inactive", "active");
      } else {
        moreCont.classList.replace("active", "inactive");
      }
    }
  });
...
function handleCheckIfSignedIn() {
  ...
  if (user) {
    ...
    signInBtn.classList =
      "py-1 lg:py-2 px-2 text-xs md:text-sm xl:text-[1rem] relative underline-parent font-semibold flex-shrink-0";
    const signInText = document.createElement("span");
    signInText.textContent = "Sign In";
    signInText.setAttribute("data-english", "Sign In");
    signInText.setAttribute("data-srpski", "Пријави се");
    signInBtn.append(signInText);
    const underline = document.createElement("div");
    underline.classList = "underline-child bg1";
    signInBtn.append(underline);
    handleUnderlineHover();
    ...
    signUpBtn.setAttribute("data-english", "Get Started");
    signUpBtn.setAttribute("data-srpski", "Почните");
  ...
  >
    ...
    p2.textContent = lang === "english" ? "Get Started" : "Почните";

    p2.setAttribute("data-english", "Get Started");
    p2.setAttribute("data-srpski", "Почните");
    li2.append(p2);
    a2.append(li2);
...
function handleSettingsDropdown() {
  const settingsJs = document.querySelector(".settings-js");
  const settingsCont = document.querySelector(".settings-container-js");
  const langSelect = document.getElementById("lang");

  function syncActiveLang() {
    const curr =
      localStorage.getItem("SolveBox-current-language") || "english";
    document.querySelectorAll(".lang-option").forEach((opt) => {
      opt.classList.toggle("text1", opt.dataset.lang === curr);
    });
  }
  ...
  window.addEventListener("localStorageChange", (e) => {
    if (e.key === "SolveBox-current-language") syncActiveLang();
  });

  settingsJs?.addEventListener("click", (e) => {
    if (!settingsCont?.contains(e.target)) {
      e.stopPropagation();
      if (settingsCont?.classList.contains("inactive")) {
        document.querySelector(".theme-container-js")?.classList.replace("active", "inactive");
        document.querySelector(".more-container-js")?.classList.replace("active", "inactive");
        settingsCont.classList.replace("inactive", "active");
      } else {
        settingsCont.classList.replace("active", "inactive");
      }
    }
  });
  ...
  document.querySelectorAll(".lang-option").forEach((opt) => {
    opt.addEventListener("click", (e) => {
      e.stopPropagation();
      if (langSelect) {
        langSelect.value = opt.dataset.lang;
